const infant=0;
  ...
  let mobilePhone='';
  let email='';
  let guestGender = "Please specify gender";
...
document.getElementById("maleGender").onclick = () => guestGender = "Male";
document.getElementById("femaleGender").onclick = () => guestGender = "Female";

// Creating letiables for validation purposes
...
document.getElementById("fullName").addEventListener('input', function () {
  if (fullName.value && /[a-zA-Z]+.*\s|\s.*[a-zA-Z]+/.test(fullName.value)) {
    validFullName = true;
  } else {
    validFullName = false;
  }
...
document.getElementById("emailAddress").addEventListener('input', function () {
  if (emailAddress.value && /^[a-zA-Z]+@[a-zA-Z]+\.[a-zA-Z]+$/.test(emailAddress.value)) {
    validEmail = true;
  } else {
    validEmail = false;
  }
  if (emailAddress.value === emailAddress2.value) {
    emailMatch = true;
  } else {
    emailMatch = false;
  }
...
document.getElementById("emailAddress2").addEventListener('input', function () {
  if (emailAddress2.value && /^[a-zA-Z]+@[a-zA-Z]+\.[a-zA-Z]+$/.test(emailAddress2.value)) {
    validEmail2 = true;
  } else {
    validEmail2 = false;
  }
...
function checkValidations() {
  if (validFullName === true && validMobileNumber === true && validEmail === true && emailMatch === true) {
    formValidated = true;
    
    document.getElementById("notice").innerHTML = `Click "Continue With Purchase " to proceed. `;
    document.getElementById("detailSubmitButton").removeAttribute('disabled');
    nameFull=fullName.value;
    mobilePhone=mobileNumber.value;
    email=emailAddress.value;  } 
    else {
    formValidated = false;
    document.getElementById("detailSubmitButton").setAttribute('disabled', '');
    let incorrectName='';
    let incorrectEmail='';
    let incorrectPhone='';
      if(validFullName===false){
        incorrectName="∙ Full Name";
      }
      if(validMobileNumber===false){
        incorrectPhone="∙ Mobile Number";
      }
      if(emailMatch===false || validEmail===false|| validEmail2===false){
        incorrectEmail="∙ E-mail";
      }
    document.getElementById("notice").innerHTML = `Please Re-check the fields: ${incorrectName} ${incorrectPhone} ${incorrectEmail}  `;
...
const saveDetails=()=>{
  ...
  localStorage.setItem('gender',guestGender);
}
...
//End of  telephone javascript code
